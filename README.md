<p align="center"><img alt="Regex" src="./assets/readme-header.png"/></p>
...
En la función anterior se esta utilizando `regex` para filtrar palabras segun un patrón establecido en la variable `patron`, dependiendo del idioma:
```kotlin
val patron = if (idioma == Idioma.ES) {
    "^[a-záéíóúüñ]+$"
} else {
...
De esta forma, filtramos para que puedan utilizarse las letras `a-z` y `áéíóúüñ` si el idioma es español:
```kotlin
.filter { it.matches(Regex(patron)) } // Solo letras
```
...
```kotlin
fun main() {
    val regex = Regex(".*ma.*", RegexOption.IGNORE_CASE)
    
    val listaPalabras = listOf("Marco", "Amapola", "Mazorca", "Matador", "Plátano", "Zapatilla")

    val listaFiltrada = listaPalabras.filtrar(regex)
